...
const Footer = ({ onNavigate }) => {
  ...
  const FooterSection = ({ id, title, children }) => {
    const isOpen = openSection === id;
    return (
      <div className="lg:px-10 flex flex-col items-start text-left border-b border-[#dcebdc]/60 lg:border-none pb-4 lg:pb-0">
        {/* Mobile Accordion Toggle */}
        <button 
          onClick={() => toggleSection(id)}
          className="w-full flex justify-between items-center lg:hidden py-4 group"
        >
          <h4 className="text-[#0a1f10] font-black text-[1.05rem] uppercase tracking-widest">{title}</h4>
          <i className={`bx bx-chevron-down text-2xl text-[#65a30d] transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`}></i>
        </button>

        {/* Desktop Header */}
        <h4 className="hidden lg:inline-block text-[#0a1f10] font-black text-[1.05rem] mb-8 uppercase tracking-widest relative after:content-[''] after:absolute after:-bottom-2.5 after:left-0 after:w-12 after:h-1 after:bg-[#65a30d] after:rounded-full">
          {title}
        </h4>

        {/* Content (Animated for Mobile, Static for Desktop) */}
  ...
  return (
    <footer className="relative bg-[#f4fbf4] border-t border-[#dcebdc] pt-16 sm:pt-20 pb-28 lg:pb-12">
      <motion.div
      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-5 lg:divide-x divide-[#dcebdc] gap-y-4 lg:gap-y-0">

          {/* Logo Section */}
          <div className="lg:col-span-1 lg:pr-10 flex flex-col items-start text-left mb-6 lg:mb-0">
            <a href="#" onClick={(e) => handleNav(e, 'home')} className="block mb-6 lg:mb-8">
              <img src="/assets/brand-logo.png" alt="Star Gardens" className="w-[200px] md:w-[260px] lg:w-[320px] h-14 md:h-16 lg:h-[80px] object-cover object-left lg:object-center" />
            </a>
            ...
            <div className="flex justify-start gap-3">
              ...
              <a href="https://www.instagram.com/reel/DYSN97qzAZI/?utm_source=ig_web_copy_link&igsh=MzRlODBiNWFlZA%3D%3D" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white border border-[#1b8b22]/20 flex items-center justify-center text-[#1b8b22] text-lg transition-all duration-300 hover:bg-[#65a30d] hover:text-white hover:shadow-lg hover:-translate-y-1"><i className='bx bxl-instagram'></i></a>
          ...
          <FooterSection id="quickLinks" title="QUICK LINKS">
          ...
          <FooterSection id="services" title="OUR SERVICES">
            <ul className="space-y-4 flex flex-col items-start">
          ...
          <FooterSection id="products" title="OUR PRODUCTS">
            <ul className="space-y-4 flex flex-col items-start">
          ...
          <FooterSection id="contact" title="CONTACT US">
            <ul className="space-y-5 flex flex-col items-start w-full">
              <li className="flex gap-4 items-start group cursor-default w-full">
                <i className='bx bx-map text-[#65a30d] text-2xl mt-0.5 group-hover:scale-110 transition-transform'></i>
              ...
              <li className="flex gap-4 items-center group cursor-pointer w-full">
                <i className='bx bx-phone text-[#65a30d] text-2xl group-hover:scale-110 transition-transform'></i>
              ...
              <li className="flex gap-4 items-center group cursor-pointer w-full">
                <i className='bx bx-globe text-[#65a30d] text-2xl group-hover:scale-110 transition-transform'></i>
        ...
        {/* Footer Tagline */}
        <div className="mt-12 lg:mt-16 pt-6 border-t border-[#dcebdc] text-center flex flex-col items-center justify-center">
